...
namespace FSMS.WebAPI.Controllers
{
    ...
    public class GardenTasksController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAllGardenTasks(string? gardenTaskName = null, DateTime? taskDate = null, bool activeOnly = false)
        {
            try
            {
                List<GetGardenTask> gardenTasks = await _gardenTaskService.GetAllAsync(gardenTaskName, taskDate, activeOnly);
                return Ok(new
                {
                    Data = gardenTasks
                });
