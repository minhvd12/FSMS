...
namespace FSMS.WebAPI.Controllers
{
    ...
    public class ReviewFruitsController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAllReviewFruits(bool activeOnly = false)
        {
            try
            {
                List<GetReviewFruit> reviewFruits = await _reviewFruitService.GetAllReviewFruitsAsync(activeOnly);
                return Ok(new
                {
                    Data = reviewFruits
                });
