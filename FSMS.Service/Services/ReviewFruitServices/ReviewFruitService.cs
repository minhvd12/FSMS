...
*/                    FruitId = createReviewFruit.FruitId,
        ...
        public async Task<List<GetReviewFruit>> GetAllReviewFruitsAsync(bool activeOnly = false)
        {
            try
            {
                var reviewFruits = (await _reviewFruitRepository.GetAsync(includeProperties: "User,Fruit"))
                    .Where(reviewProduct => !activeOnly || reviewProduct.Status == StatusEnums.Active.ToString());
